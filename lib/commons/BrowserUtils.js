...
(function (BrowserUtils) {
     ...
     */
    function checkElement(elementSelector, imageFileName, options = {}) {
        Reporter_1.Reporter.step('Compare');
        waitForDisplayed(elementSelector);
        Reporter_1.Reporter.debug(`Compare element '${imageFileName}' with selector '${elementSelector}' and options ${options}`);
        // @ts-ignore
        const compareResult = browser.checkElement($(elementSelector), imageFileName, options);
        if (compareResult !== 0) {
            throw new Error(`Found ${compareResult}% difference. See attached images`);
        }
    }
    BrowserUtils.checkElement = checkElement;
    /**
    ...
    function clearValue(selector) {
        Reporter_1.Reporter.debug(`Clear text in '${selector}'`);
        waitForDisplayed(selector);
     ...
     */
    function waitUntil(action, errMessage, actionTimeout = DEFAULT_TIME_OUT) {
    ...
    function isDisplayed(selector) {
        Reporter_1.Reporter.debug(`Is element visible by '${selector}'`);
    ...
    function notExist(notExistElementSelector) {
        ...
        tryBlock(() => $(notExistElementSelector).waitForExist({
            timeout: DEFAULT_TIME_OUT,
            reverse: true,
    ...
    function isIframeVisible(iframeSelector, expectedVisibility) {
        ...
        if (iframeVisibility !== expectedVisibility) {
            chai_1.assert.fail(`Failed on iframe '${iframeSelector}' visibility validation. ${os_1.EOL} Expected: '${expectedVisibility}' ${os_1.EOL} Actual: '${iframeVisibility}'`);
        }
    }
    ...
    function getAttributeAndVerify(selector, attributeName) {
        ...
        if (stringResults === null || stringResults === undefined) {
            chai_1.assert.fail(`Found multiple results matching requested attribute '${attributeName}' or no results for element: '${selector}'`);
        }
    ...
    function sendKeys(keysToSend) {
        Reporter_1.Reporter.debug(`Sending Keys ${keysToSend}`);
    ...
    function verifyFilesInZip(linkToZipFile, listOfFileNames, expectedNumOfFiles) {
        ...
        if (expectedNumOfFiles !== undefined && expectedNumOfFiles !== zipFileNames.length) {
            ...
            chai_1.assert.fail(incorrectLengthErrorMessage);
        }
        if (!listOfFileNames.every((fileName) => zipFileNames.includes(fileName))) {
            ...
            chai_1.assert.fail(incorrectListErrorMessage);
        }
    }
