...
class EyesUtil {
    ...
    checkWithIgnores(checkDescription, xPaths) {
        ...
        browser.call(() => {
            const tr = this.eyes.check(checkDescription, targetWindowObj).getResult();
            Reporter_1.Reporter.debug(`TEST RESULT: ${util_1.inspect(tr)}`);
            return tr;
        });
    }
    /**
    ...
    checkPageLayout(checkDesc) {
        ...
        return browser.call(() => {
            const tr = this.eyes.check(checkDesc, eyes_webdriverio_1.Target.window().layout()).getResult();
            Reporter_1.Reporter.debug(`TEST RESULT: ${util_1.inspect(tr)}`);
            return tr;
        });
    }
