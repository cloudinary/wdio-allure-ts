...
const TIMEOUT = 4000; // default 2000
let result;
/**
...
class EyesUtil {
     ...
     */
    open(applicationName, testName, boundingBoxObj) {
        Reporter_1.Reporter.debug('Open eyes');
        browser.call(() => {
            return this.eyes.open(browser, testName, applicationName, boundingBoxObj === undefined ? { width: SCREEN_WIDTH, height: SCREEN_HEIGHT } : boundingBoxObj);
        });
    ...
    checkWithIgnores(checkDescription, xPaths) {
        ...
        Reporter_1.Reporter.debug(`TEST RESULT: ${util_1.inspect(result._asExpected)}`);
    ...
    checkPageLayout(checkDesc) {
        ...
        Reporter_1.Reporter.debug(`TEST RESULT: ${result._asExpected}`);
        return result._asExpected;
    }
    /**
     * Close eye batch
