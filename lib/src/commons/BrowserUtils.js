...
(function (BrowserUtils) {
    ...
    function sendText(selector, text) {
        ...
        isVisible(selector);
        const element = $(selector);
        tryBlock(() => element.addValue(text), `Failed to add text:${text} to ${selector}`);
    }
    ...
    function doubleClick(selector) {
        ...
        Reporter_1.Reporter.debug('SUCCESS: Double Click.');
        Reporter_1.Reporter.debug('********************************************************************************************');
    }
    ...
    function expectCurrentUrl(url) {
        ...
        }, 30000, `Url not as expected.${os_1.EOL} Expected: '${expectedUrl}' ${os_1.EOL} Actual: '${currentUrl}'`);
    }
    ...
    function selectByValue(selector, value) {
        ...
        Reporter_1.Reporter.debug(`SUCCESS: [${value}] Selected.`);
        Reporter_1.Reporter.debug('********************************************************************************************');
    }
    ...
    function isVisible(selector) {
        ...
        Reporter_1.Reporter.debug('SUCCESS: Element is visible.');
    }
    ...
    function isExist(selector) {
        ...
        Reporter_1.Reporter.debug('SUCCESS: Element exists.');
    }
    ...
    function notVisible(notVisibleElementSelector) {
        ...
        Reporter_1.Reporter.debug('SUCCESS: Element is not visible.');
    }
    ...
    function notExist(notExistElementSelector) {
        ...
        Reporter_1.Reporter.debug('SUCCESS: Element not exists.');
    }
    ...
    function switchToFrame(iframeSelector) {
        ...
        chillOut();
        Reporter_1.Reporter.debug('Switching to Iframe');
        isExist(iframeSelector);
        Reporter_1.Reporter.debug(`Switching to Iframe [${iframeSelector}]`);
        tryBlock(() => browser.switchToFrame(iframeSelector), 'Failed to switch frame');
        chillOut();
        Reporter_1.Reporter.debug('SUCCESS: Switch to iFrame.');
        Reporter_1.Reporter.debug('********************************************************************************************');
    }
    ...
    function switchTab(handle) {
        ...
        Reporter_1.Reporter.debug(`SUCCESS: Switching tab to [${handle}]`);
    }
    ...
    function switchToParentFrame() {
        ...
        Reporter_1.Reporter.debug('SUCCESS: Switch to parent frame');
    }
     ...
     */
    function expectText(selectorType, selector, text) {
        Reporter_1.Reporter.debug('********************************************************************************************');
        Reporter_1.Reporter.debug(`Validate element text is [${text}] by selector [${selector}]`);
        isVisible(selector);
        const foundText = getText(selectorType, selector);
        ...
        Reporter_1.Reporter.debug(`SUCCESS: Text [${foundText}] as expected.`);
        Reporter_1.Reporter.debug('********************************************************************************************');
    }
     ...
     */
    function getText(selectorType, selector) {
        Reporter_1.Reporter.debug(`Get element's text by selector [${selector}]`);
        return tryBlock(() => getTextAndVerify(selectorType, selector), `Failed to get text from element '${selector}'`);
    }
     ...
     * @param selector - element locator
     */
    function getTextAndVerify(selectorType, selector) {
        Reporter_1.Reporter.debug('********************************************************************************************');
        Reporter_1.Reporter.debug(`Get Text & Verify Not Null, [${selector}]`);
        const element = $(selector);
        const stringResults = findElements(selectorType, selector).length === 1
            ? element.getText()
            : undefined;
        ...
        Reporter_1.Reporter.debug(`SUCCESS: Get Text Result: [${stringResults}]`);
        Reporter_1.Reporter.debug('********************************************************************************************');
     ...
     * @param selector - element locator
     */
    function expectNumberOfElements(selectorType, selector, expectedValue) {
        Reporter_1.Reporter.debug('********************************************************************************************');
        ...
        Reporter_1.Reporter.debug('SUCCESS: Expect Number Of Elements.');
        Reporter_1.Reporter.debug('********************************************************************************************');
    }
    ...
    function isIframeVisible(iframeSelector, expectedVisibility) {
        ...
        if (iframeVisibility !== expectedVisibility) {
            throw new Error(`Failed on iframe ${iframeSelector} visibility validation. ${os_1.EOL} Expected: ${expectedVisibility}, actual: ${iframeVisibility} ${os_1.EOL}`);
        }
        Reporter_1.Reporter.debug(`SUCCESS: iFrame visibility is [${iframeVisibility}].`);
        Reporter_1.Reporter.debug('********************************************************************************************');
    }
     ...
     */
    function getAttribute(selectorType, selector, attributeName) {
        return tryBlock(() => getAttributeAndVerify(selectorType, selector, attributeName), `Failed to get ${attributeName} attribute from ${selector}`);
    }
     ...
     * @param attributeName attribute name to search for
     */
    function getAttributeAndVerify(selectorType, selector, attributeName) {
        Reporter_1.Reporter.debug('********************************************************************************************');
        ...
        Reporter_1.Reporter.debug(`SUCCESS: Attribute result [${stringResults}].`);
        Reporter_1.Reporter.debug('********************************************************************************************');
        return stringResults;
    }
    /**
     ...
     */
    function expectAttributeValue(selectorType, selector, attributeName, value) {
        Reporter_1.Reporter.debug('********************************************************************************************');
        ...
        Reporter_1.Reporter.debug(`SUCCESS: Attribute value as expected.`);
        Reporter_1.Reporter.debug('********************************************************************************************');
    }
     ...
     */
    function expectNoAttributeValue(selectorType, selector, attributeName, value) {
        let attributeValue;
        Reporter_1.Reporter.debug('********************************************************************************************');
        ...
        Reporter_1.Reporter.debug(`SUCCESS: Attribute value wasn't found as expected.`);
        Reporter_1.Reporter.debug('********************************************************************************************');
    }
    ...
    function acceptAlert() {
        ...
        Reporter_1.Reporter.debug('SUCCESS: Accept alert');
    }
    ...
    function dismissAlert() {
        ...
        Reporter_1.Reporter.debug('SUCCESS: Dismiss alert');
    }
    ...
    function expectAlertText(expectedText) {
        ...
        Reporter_1.Reporter.debug(`SUCCESS: Alert's text. [${os_1.EOL}] Expected: [${expectedText}] [${os_1.EOL}] Actual: [${actualText}]`);
        Reporter_1.Reporter.debug('********************************************************************************************');
    }
    ...
    function setViewportSize(wSize) {
        Reporter_1.Reporter.debug('********************************************************************************************');
        Reporter_1.Reporter.debug(`Requested ViewPort size [${wSize.width}px X ${wSize.height}px]`);
        ...
        Reporter_1.Reporter.debug('SUCCESS: Viewport size changed.');
        Reporter_1.Reporter.debug('********************************************************************************************');
    }
