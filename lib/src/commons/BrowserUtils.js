...
(function (BrowserUtils) {
    ...
    function isExist(selector) {
        ...
        console.log(`$(${selector}).waitForExist(DEFAULT_TIME_OUT)`);
        browser.debug();
    ...
    function expectText(selector, expectedText) {
        ...
        isExist(selector);
        let foundText = getTextAndVerify(selector).replace(/(\n)/gm, ' ');
        tryBlock(() => browser.waitUntil(() => {
            if (foundText.trim().toLowerCase() === expectedText.trim().toLowerCase()) {
                return true;
            }
