...
(function (BrowserUtils) {
    ...
    function expectCurrentUrl(url) {
        ...
        }, 30000, `Incorrect URL. Expected ${expectedUrl} while actual: ${browser.getUrl()}`);
    }
    ...
    function expectText(selector, text) {
        ...
        isVisible(selector);
        const currText = tryBlock(() => browser.getText(selector), `Failed to get text from element '${selector}'`).replace(/(\n)/gm, " "); // replace EOL with space, for more readable tests strings;
        if (currText !== text) {
