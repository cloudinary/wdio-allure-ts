...
(function (BrowserUtils) {
    ...
    function verifyFilesInZip(linkToZipFile, listOfFileNames, expectedNumOfFiles) {
        ...
        const zipFileNames = zipToBuffer(linkToZipFile);
        if (expectedNumOfFiles !== undefined &&
            expectedNumOfFiles !== zipFileNames.length) {
            const incorrectLengthErrorMessage = `Incorrect number of files. Expected '${JSON.stringify(expectedNumOfFiles)}', actual '${JSON.stringify(zipFileNames.length)}'`;
    ...
    function zipToBuffer(linkToZipFile) {
