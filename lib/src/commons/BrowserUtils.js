...
(function (BrowserUtils) {
    ...
    function getTextAndVerify(selector) {
        // @ts-ignore
        const stringResults = browser.getText(selector);
        //Check for multiple results
        if (stringResults.length > 1) {
            throw new Error(`Found multiple results matching text for element: '${selector}'`);
        }
