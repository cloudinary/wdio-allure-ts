...
export function expectNoAttributeValue(selector: string, attributeName: string, value: string): void {
  let attributeValue: string;
  Reporter.debug(`Validate element '${selector}' doesn't have attribute '${attributeName}' which contains '${value}'`);
  tryBlock(
    () =>
      browser.waitUntil(() => {
        attributeValue = getAttribute(selector, attributeName);
...
export function compareWithSnapshot(filename: string, selector: string, options: object = {}): void {
  ...
  if (compareResult !== 0) {
    ...
    allureReporter.addAttachment('Base line', fs.readFileSync(path.join(baselinePath, filename + '.png')), 'image/png');
