...
describe('ViewportSizeSpec of BrowserUtils Tests', () => {
    ...
    it('Validate window size after size change to 1024x768', () => {
        ...
        BrowserUtils.setViewportSize({ width: DEFAULT_WIDTH, height: DEFAULT_HEIGHT });
        browser.pause(1000);
        assert.equal(browser.execute(()=> window.innerWidth).value, DEFAULT_WIDTH, 'window width');
        assert.equal(browser.execute(()=> window.innerHeight).value, DEFAULT_HEIGHT, 'window height');
    });
