...
const linkToZip: string =
  'http://127.0.0.1:8000/test/resources/archiveWithFiles.zip';
const fileNamesList: string[] = [
  'firstFile.txt',
  'secondFile.jpeg',
  'innerFolder/thirdFile.doc',
];
/**
 * verifyFilesInZip
 */
describeCommon('verifyFilesInZip', () => {
  it('correct list without size', () => {
    expect(() =>
      BrowserUtils.verifyFilesInZip(linkToZip, fileNamesList)
    ).to.not.throw(Error);
  });
  ...
  it('correct list with size', () => {
    expect(() =>
      BrowserUtils.verifyFilesInZip(linkToZip, fileNamesList, 3)
    ).to.not.throw(Error);
  });
